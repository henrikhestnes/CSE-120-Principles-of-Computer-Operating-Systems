 ...
 * road, but since they enter from opposing directions, they will eventually
 ...
 *   	direction), and if it enters it will crash (breaking rule A);
...
void Main()
{
	 ...
	 * modifications of driveRoad.  When your solution is tested, we
...
void InitRoad()
{
	...
	shm.countMtx = 	Seminit(1);
	shm.countWestMtx = Seminit(1);
	shm.countEastMtx = Seminit(1);
	int p;
	for(p = 0; p < NUMPOS; p++){
		shm.posMtx[p] = Seminit(1);
	}
	shm.roadReady = Seminit(1);
}
...
void driveRoad(int from, int mph)
	...
	// mph: speed of the car
{
	Wait(shm.countMtx);
		if(from == WEST){
			Wait(shm.countWestMtx);
			shm.countWest++;
			if(shm.countWest == 1){
				Wait(shm.roadReady);
			}
			Signal(shm.countWestMtx);
		} else{// From east
			Wait(shm.countEastMtx);
			shm.countEast++;
			if(shm.countEast == 1){
				Wait(shm.roadReady);
			}
			Signal(shm.countEastMtx);
		}
	Signal(shm.countMtx);
	...
	if(from == WEST){
		...
		Wait(shm.posMtx[NUMPOS - 1]);
			EnterRoad(from);
	}


	int i, nextPos, pos;
	for (i = 1; i < NUMPOS; i++) { //Allowed to change p?
		...
		Wait(shm.posMtx[nextPos]);
			ProceedRoad();
		Signal(shm.posMtx[pos]);
	}
	
	ProceedRoad();
	Signal(shm.posMtx[nextPos]);
